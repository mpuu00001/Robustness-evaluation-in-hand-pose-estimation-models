...
def document_evaluation_metric(input_dataframe, output_matrix_dir, output_result_dir, result_filename, type, threshold,
                               is_hand_recognition,
                               group_by=None):
    """
    ...
    if group_by != 'keypoint':
        ...
        if is_hand_recognition:
            confusion_matrix = form_confusion_matrix_for_hand_recognition_results(input_dataframe, 'iou', threshold,
                                                                                  True)
        else:
            confusion_matrix = form_confusion_matrix_for_hand_landmark_localisation_results(input_dataframe, ['iou',
                                                                                                              'mean_keypoint_error'],
                                                                                            threshold)
        ...
        # Compute confusion matrix:
        confusion_matrix = form_confusion_matrix_for_hand_landmark_localisation_results(input_dataframe,
                                                                                        ['iou', 'mean_keypoint_error'],
                                                                                        threshold, 'keypoint')
...
def document_preparation(main_output_dir, result_filename, result_header):
    ...
    write_csv_data(output_hand_landmark_localisation_result_by_keypoint_path + result_filename,
                   result_by_keypoint_header, 'w')


def document_single_occlusion_results(source_dir, main_output_dir, result_filename, result_header, type, combine=False,
                                      dataset='FreiHand', size=244):
    # Read data
    ...
    document_evaluation_metric(input_dataframe=dataframe, output_matrix_dir=output_hand_recognition_matrix_path,
                               output_result_dir=output_hand_recognition_result_path, result_filename=result_filename,
                               type=type,
    ...
    output_hand_landmark_localisation_result_path = main_output_dir + 'overall/hand_landmark_localisation/'
    output_hand_landmark_localisation_matrix_path = main_output_dir + 'confusion_matrixes/hand_landmark_localisation/'
    if dataset == 'FreiHand' or 'CMUhand':
    ...
    if size != 244:
        ...
        print('hand_localisation_threshold' + str(hand_localisation_threshold))

    document_evaluation_metric(input_dataframe=dataframe,
                               output_matrix_dir=output_hand_landmark_localisation_matrix_path,
                               output_result_dir=output_hand_landmark_localisation_result_path,
...
def document_baseline_results_naked_hands(dataset='FreiHand'):
    source_path = r'result/' + dataset + r'/baseline/raw/naked_hands_prediction_result.csv'
    main_output_dir = r'result/' + dataset + r'/baseline/'
    ...
    document_single_occlusion_results(source_dir=source_path, main_output_dir=main_output_dir,
                                      ...
                                      type='naked_hands', combine=True, dataset='FreiHand')


def document_baseline_results_filtered_naked_hands(dataset='FreiHand'):
    source_path = r'result/' + dataset + r'/baseline/raw/filtered_naked_hands_prediction_result.csv'
    main_output_dir = r'result/' + dataset + r'/baseline/'
    ...
    document_single_occlusion_results(source_dir=source_path, main_output_dir=main_output_dir,
                                      result_filename=result_filename, result_header=result_header,
                                      type='filtered_naked_hands', combine=True, dataset='FreiHand')

def document_baseline_results_filtered_hands_with_objects(dataset='FreiHand'):
    source_path = r'result/' + dataset + r'/baseline/raw/filtered_hands_with_objects_prediction_result.csv'
    main_output_dir = r'result/' + dataset + r'/baseline/'

    result_header = ['test_case', 'precision', 'recall', 'f1_score']
    result_filename = 'filtered_hands_with_objects.csv'
...
def document_baseline_results_hands_with_objects(dataset='FreiHand'):
    source_path = r'result/' + dataset + r'/baseline/raw/hands_with_objects_prediction_result.csv'
    main_output_dir = r'result/' + dataset + r'/baseline/'
    ...
    document_single_occlusion_results(source_dir=source_path, main_output_dir=main_output_dir,
                                      result_filename=result_filename, result_header=result_header,
                                      type='hands_with_objects', combine=True, dataset='FreiHand')


def document_regional_occlusion_results(dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/regional_occlusion/raw/'
    main_output_dir = r'result/' + dataset + r'/regional_occlusion/'
    ...
    for i in range(len(region_labels)):
        ...
        document_single_occlusion_results(source_dir=new_source_path, main_output_dir=main_output_dir,
                                          result_filename=result_filename, result_header=result_header,
                                          type=new_type, combine=True, dataset='FreiHand')


def document_finger_occlusion_results(dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/finger_occlusion/raw/'
    main_output_dir = r'result/' + dataset + r'/finger_occlusion/'
...
def document_level_occlusion_results(occluded_by='black_circle', dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/level_occlusion/raw/' + occluded_by + '/'
    main_output_dir = r'result/' + dataset + r'/level_occlusion/'
...
def document_different_exposure_rate_results(dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/exposure_rate/raw/'
    main_output_dir = r'result/' + dataset + r'/exposure_rate/'

    result_header = ['test_case', 'precision', 'recall', 'f1_score']
    result_filename = 'finger_occlusion_results.csv'
    document_preparation(main_output_dir, result_filename, result_header)

    labels = ['strongly_underexposed', 'underexposed', 'overexposed', 'strongly_overexposed']
    for i in range(len(labels)):
        new_source_dir = source_dir + f'{labels[i]}_hands_prediction_result.csv'
        new_type = labels[i]
        document_single_occlusion_results(source_dir=new_source_dir, main_output_dir=main_output_dir,
                                          result_filename=result_filename, result_header=result_header,
                                          type=new_type, combine=True, dataset='FreiHand')
...
def document_different_motion_blur_results(dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/motion_blur/raw/'
    main_output_dir = r'result/' + dataset + r'/motion_blur/'
    ...
    for i in range(len(labels)):
        new_source_dir = source_dir + f'{labels[i]}_motion_blurred_hands_prediction_result.csv'
        new_type = labels[i]
        document_single_occlusion_results(source_dir=new_source_dir, main_output_dir=main_output_dir,
                                          result_filename=result_filename, result_header=result_header,
                                          type=new_type, combine=True, dataset='FreiHand')
...
def document_different_scaling_factor(dataset='FreiHand'):
    source_dir = r'result/' + dataset + r'/scale/raw/'
    main_output_dir = r'result/' + dataset + r'/scale/'
    ...
    labels = ['122', '488']
    for i in range(len(labels)):
        new_source_dir = source_dir + f'{labels[i]}_sized_hands_prediction_result.csv'
        new_type = labels[i]
        document_single_occlusion_results(source_dir=new_source_dir, main_output_dir=main_output_dir,
                                          result_filename=result_filename, result_header=result_header,
                                          type=new_type, combine=True, dataset='FreiHand', size=int(labels[i]))
